...
const DashboardSolicitante: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!itemId) throw new Error("Falha ao recuperar o ID do item criado.");

      const nfId = formData.invoiceNumber?.trim() || `ID_${itemId}`;
      const baseName = `NF_${nfId}`;

      // Upload na Lista Principal (GUID fornecido)
      ...
      // Upload na Lista Auxiliar (GUID fornecido)
      if (ticketFiles.length > 0) {
        setUploadStatus('Gerando registro de anexos...');
        ...
        for (let i = 0; i < ticketFiles.length; i++) {
          ...
          setUploadStatus(`Enviando Doc ${i + 1}/${ticketFiles.length}...`);
      ...
      <div className="w-96 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-lg font-black text-gray-800 uppercase tracking-tight">Meus Lançamentos</h1>
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Pesquisar notas..." className="w-full pl-10 pr-4 py-3 bg-gray-50 border-0 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
          ...
          {filteredRequests.map(req => (
            <button key={req.id} onClick={() => { setSelectedId(req.id); setIsNew(false); }} className={`w-full p-4 rounded-2xl transition-all text-left border-2 ${selectedId === req.id ? 'bg-blue-50 border-blue-600' : 'bg-white border-transparent hover:bg-gray-50 shadow-sm'}`}>
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-100 px-2 py-0.5 rounded-md">ID: {req.id}</span>
              ...
              <div className="flex items-center text-[10px] font-bold text-gray-400 uppercase tracking-widest space-x-3">
                ...
                <span className="text-blue-500">{req.branch}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className={`flex-1 flex flex-col transition-all ${isNew ? 'bg-slate-900 text-white' : 'bg-gray-50'}`}>
        {isNew ? (
          <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
            <div className="max-w-4xl mx-auto space-y-10">
              <header className="flex items-center justify-between border-b border-white/10 pb-8">
                <h2 className="text-3xl font-black uppercase italic tracking-tight">{isEditing ? 'Ajustar Dados' : 'Nova Solicitação'}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Descrição do Pagamento *</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Número da Nota Fiscal</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Forma de Pagamento</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Vencimento da Fatura</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Nº dos Pedidos / Ordens</label>
              ...
              {(formData.paymentMethod === 'TED/DEPOSITO' || formData.paymentMethod === 'PIX') && (
                <div className="bg-white/5 p-8 rounded-2xl border border-white/5 space-y-6">
                  <h3 className="text-xs font-black uppercase tracking-widest text-blue-400 flex items-center italic"><Landmark size={18} className="mr-2" /> Dados para Recebimento</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2">
                       <label className="block text-[10px] font-black uppercase text-white/40 mb-2 tracking-widest">Favorecido</label>
              ...
              <div className="md:col-span-2">
                <label className="block text-[10px] font-black uppercase text-blue-400 mb-2 tracking-widest">Observações Adicionais</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white/5 p-8 rounded-2xl border border-white/5 space-y-4">
                  <h3 className="text-xs font-black uppercase tracking-widest text-blue-400 italic flex items-center"><CreditCard size={18} className="mr-2" /> Boletos / Docs</h3>
                  <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-xl p-8 hover:bg-white/10 cursor-pointer transition-all">
                    <Paperclip size={32} className="text-blue-400 mb-2 opacity-20" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Anexar Documento</span>
            ...
            <header className="p-12 border-b border-gray-200 bg-white flex justify-between items-end shadow-sm">
              <div>
                <div className="flex items-center space-x-3 mb-4">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] bg-gray-50 px-3 py-1 rounded-md">Protocolo: #{selectedRequest.id}</span>
            ...
            <div className="flex-1 overflow-y-auto p-12 space-y-8 custom-scrollbar">
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 italic">Dados da Nota</p>
                  <div className="grid grid-cols-2 gap-6">
                    <div><span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Número</span><p className="text-xl font-black text-gray-900 break-words">{stripHtml(selectedRequest.invoiceNumber) || '---'}</p></div>
                    <div><span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Vencimento</span><p className="text-xl font-black text-gray-900">{new Date(selectedRequest.paymentDate).toLocaleDateString()}</p></div>
                  </div>
                </div>
                <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <p className="text-xs font-bold text-gray-500 mt-2 uppercase tracking-tight truncate">{selectedRequest.payee || '---'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-300 text-center animate-in fade-in">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-2 max-w-xs">Selecione uma solicitação no painel lateral para ver os detalhes.</p>
          </div>
        )}
      </div>
    </div>
