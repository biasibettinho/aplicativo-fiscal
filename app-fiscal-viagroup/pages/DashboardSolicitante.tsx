...
import { 
  Plus, Search, History, ChevronRight, Clock, Loader2, Calendar, User, CreditCard, Hash, Landmark, Info, Edit3, Send
} from 'lucide-react';
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && selectedId) {
        ...
        // Regra de Reenvio:
        // Erro Fiscal -> Volta para Pendente (Fiscal)
        // Erro Financeiro -> Volta para Aprovado (Financeiro)
        if (currentReq?.status === RequestStatus.ERRO_FISCAL) {
          nextStatus = RequestStatus.PENDENTE;
        } else if (currentReq?.status === RequestStatus.ERRO_FINANCEIRO) {
        ...
        await requestService.updateRequest(selectedId, {
          ...formData,
          status: nextStatus,
          errorType: '', // Limpa o erro ao reenviar
          errorObservation: ''
        }, authState.token);
      } else {
        ...
        <div className="w-1/3 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50/50">
            <div className="flex items-center justify-between mb-4">
              ...
              <button 
                onClick={() => { setIsNew(true); setIsEditing(false); setSelectedId(null); setFormData(initialFormData); }}
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
            {filteredRequests.map(req => (
              <div 
                key={req.id}
                onClick={() => { setSelectedId(req.id); setIsNew(false); setIsEditing(false); }}
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center space-x-2">
                    ...
                    {hasError(req.status) && (
                      <div className="text-blue-500 animate-pulse" title="Clique para ver o motivo do erro">
                        <Info size={14} />
                      </div>
                    )}
                  </div>
          ...
          {isNew ? (
            <div className="flex-1 overflow-y-auto p-10">
              <div className="max-w-3xl mx-auto space-y-6">
                <header className="flex items-center justify-between mb-4 pb-2 border-b border-blue-200">
                ...
                <div className="mt-8 flex justify-end space-x-4">
                  <button onClick={() => { setIsNew(false); setIsEditing(false); }} className="px-6 py-3 font-bold text-gray-500 uppercase text-xs">Cancelar</button>
                  <button 
                    onClick={handleSave} 
                    disabled={!isFormValid || isLoading} 
                    className={`px-8 py-3 ${isEditing ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white rounded-xl font-black uppercase text-xs disabled:opacity-50 transition-all flex items-center`}
                  >
                    {isLoading ? <Loader2 className="animate-spin mr-2" /> : (isEditing ? <Send size={16} className="mr-2" /> : <Plus size={16} className="mr-2" />)}
                    {isEditing ? 'Reenviar para Aprovação' : 'Criar Solicitação'}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col">
              <header className="p-8 border-b border-gray-100 flex justify-between items-center bg-white">
                ...
                {hasError(selectedRequest.status) && (
                  <button 
                    onClick={startEdit}
                    className="flex items-center space-x-2 bg-blue-50 text-blue-600 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-100 transition-all border border-blue-200"
                  >
                    <Edit3 size={18} />
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-6">
                {(selectedRequest.status === RequestStatus.ERRO_FISCAL || selectedRequest.status === RequestStatus.ERRO_FINANCEIRO) && (
                  <div className="bg-red-50 p-6 rounded-2xl border border-red-200 shadow-sm animate-in slide-in-from-top-4 duration-300">
                    <div className="flex items-center space-x-3 mb-3 text-red-600">
                      <Info size={20} />
                      <h4 className="text-sm font-black uppercase tracking-widest">Atenção: Correção Necessária</h4>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs font-black text-red-900 uppercase">Motivo: <span className="font-bold normal-case text-red-800">{selectedRequest.errorType || 'Não especificado'}</span></p>
                      <p className="text-sm text-red-700 italic mt-2 bg-white/50 p-3 rounded-lg border border-red-100">
                        "{selectedRequest.errorObservation || 'O analista não deixou observações detalhadas.'}"
                      </p>
                    </div>
                  </div>
                )}
