...
// Exportando MAIN_LIST_ID para que seja acessível em outros arquivos
export const MAIN_LIST_ID = '51e89570-51be-41d0-98c9-d57a5686e13b';
 ...
 */
const mapSharePointItem = (item: any): PaymentRequest => {
    const f = item.fields || {};
    ...
    return {
        id: String(numericId),
        graphId: item.id,
        mirrorId: parseInt(String(numericId ?? '0'), 10),
...
export const sharepointService = {
    getRequests: async (accessToken: string): Promise<PaymentRequest[]> => {
        try {
            let allItems: any[] = [];
            let nextLink: string | null = `https://graph.microsoft.com/v1.0/sites/${GRAPH_SITE_ID}/lists/${MAIN_LIST_ID}/items?$expand=fields($select=id,Title,Filial,Status,FAVORECIDO,BANCO,AGENCIA,CONTA,TIPO_CONTA,DESCONTO,Message_ID,Qualon_x00fa_merodaNF_x003f_,Qualopedido_x0028_s_x0029__x003f,Observa_x00e7__x00e3_o,Or_x00e7_amento,Datafinaliza_x00e7__x00e3_o,MET_PAGAMENTO,CAMPO_PIX,DATA_PAG,STATUS_ESPELHO_MANUAL,STATUS_FINAL,OBS_ERRO,OBS_CRIACAO,Dataenvio_fiscal,Dataenvio_financeiro,TESTE,comentario_compartilhamento,PESSOA_COMPARTILHADA,PESSOA_COMPARTILHOU,SolicitanteNome,SOLICITANTE_ID,SOLICITANTE_EMAIL)&$top=500`;
            ...
            return allItems.map((item: any) => mapSharePointItem(item));
        } catch (error) {
    ...
    getRequestsDelta: async (accessToken: string, lastCheck: Date): Promise<PaymentRequest[]> => {
        try {
            const isoDate = lastCheck.toISOString();
            // Filtrando por itens modificados após o último check
            const endpoint = `https://graph.microsoft.com/v1.0/sites/${GRAPH_SITE_ID}/lists/${MAIN_LIST_ID}/items?$expand=fields&$filter=lastModifiedDateTime gt '${isoDate}'`;
            
            const response = await graphFetchWithRetry(endpoint, accessToken);
            if (!response.ok) return [];
            
            const data = await response.json();
            const items = data.value || [];
            return items.map((item: any) => mapSharePointItem(item)); 
        } catch (e) {
            console.error("Erro na busca delta:", e);
            return [];
        }
    },
